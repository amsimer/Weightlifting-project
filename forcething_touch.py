...
def calibrate():
    motor.axis0.requested_state = AXIS_STATE_FULL_CALIBRATION_SEQUENCE

def turn_on():

    if(motor.axis0.current_state != AXIS_STATE_CLOSED_LOOP_CONTROL):
...
def clear_errors():
    motor.axis0.clear_errors()

def set_force_kg(kg):
    motor.axis0.motor.config.current_lim = kg*kg_to_current

def move_to(position):
    motor.axis0.controller.input_pos = position

def get_input_position():
    return motor.axis0.controller.input_pos

def get_current_position():
    return motor.axis0.encoder.pos_estimate

def get_max_force():
    return motor.axis0.motor.config.current_lim / kg_to_current

def get_current_force():
    return motor.axis0.motor.current_control.Iq_measured / kg_to_current
...
def move_increment(increment):
    motor.axis0.controller.input_pos = motor.axis0.controller.input_pos + (increment/(3*np.pi*(16.0/85.0)))

def add_weight(sender):
    print(sender)
    dpg.get_value("kg_input_double")
    set_force_kg(get_set_force()+(float(sender)/1000.0))
...
with dpg.window(label="Status", tag="win"):
    ...
    with dpg.group(horizontal=True):
        dpg.add_button(label="-10 ", callback=add_weight, tag="-10000")
        dpg.add_button(label="-5.0", callback=add_weight, tag="-5000")
        dpg.add_button(label="-1.0", callback=add_weight, tag="-1000")
        dpg.add_button(label="-.50", callback=add_weight, tag="-500")
        dpg.add_button(label="-.25", callback=add_weight, tag="-250")
        dpg.add_button(label="+.25", callback=add_weight, tag="+250")
        dpg.add_button(label="+.50", callback=add_weight, tag="+500")
        dpg.add_button(label="+1.0", callback=add_weight, tag="+1000")
        dpg.add_button(label="+5.0", callback=add_weight, tag="+5000")
